...
function onFormSubmit(e) {
  ...
  for (var i = 1; i < responses.length; i++) { // Preskočíme časovú pečiatku
    ...
    */
    if (value === "Áno" || currentCol - 1 === 12 && value === "Nie") {
    ...
    var stayDuration = responses[9]
    if (stayDuration.includes("kratší čas") || stayDuration.includes("Kratší čas")) {
      try {
        targetSheet.getRange(currentRow, 14).setValue("VYRIESIT").setBackground("#ff4040");
        try {
  ...
  /*
  if (responses[9] === "Kratší čas (do poznámky uveďte termín)") {
    try {
      targetSheet.getRange(currentRow, 14).setValue("VYRIESIT").setBackground("#ff4040");
      try {
        sendEmailShortenedTime(mail)
